...
| `apply-window` | El ciclo destructivo completo por las ventanas reales: nada se mueve sin dry-run vigente, la hoja lista exactamente lo que se mueve, **un archivo que cambió desde el escaneo se deja en paz**, aplicar consume la autorización, y deshacer devuelve byte-idéntico. Y la línea de progreso: arranca oculta y el renderizador da **tres frases distintas** para las tres etapas y **nada** para `done` | cuatro roturas: sin comparar digests, simular sin avanzar el flujo, aplicar sin consumir la autorización, `removalPlan` incluyendo grupos sin decidir , más devolver la frase de mover para verificar, o una frase para `done` |
...
| `similar-window` | El detector perceptual de punta a punta: la ventana lo ofrece con umbral **en bits** y casilla de video, un escaneo real encuentra los pares (imagen y video) y vuelve del store, un **segundo escaneo lo sirve la caché**, la biblioteca lo lista con su pie y su estado vacío correctos, el watcher nota un tercer documento, el visor pone **dos archivos distintos** lado a lado con el consejo, y **un clic decide un par y escribe una clave mientras un saltar no escribe ninguna** | siete roturas, entre ellas: `file_a` en los dos paneles, la sesión que no guarda, el estado vacío leyendo el arreglo exacto, el encabezado de imagen en un par de video, la caché apagada, decidir todos los pares en vez del seleccionado, y saltar que confirma, y **filtrar no decide nada** mientras aceptar los mostrados decide exactamente esos —con una ventana reabierta que rehidrata la decisión guardada y deja el saltado sin decidir—. Y la **línea de metadata**: dice el tamaño y la fecha de inmediato, la resolución cuando la sonda responde, y respeta el toggle sin perder lo que decía | nueve roturas, entre ellas: `confirmShown` pasando todos los índices (`accepting an empty list decided 2 pairs`) y la lista visible ignorando el filtro (`an impossible filter still shows pairs`) , más tres: quitar la asignación de la línea, no pasarle las facts al panel, y hacer que ignore la preferencia |
| `similar-apply` | Aplica una decisión perceptual a la **Papelera real** y la deshace: la compuerta rehúsa sin simulación vigente **y** con una huella que no es la mostrada, el par se re-puntúa antes de mover, el journal guarda el digest **calculado al mover**, el undo restaura byte-idéntico, y un par que dejó de parecerse se rehúsa. No deja nada en la Papelera | tres roturas: quitar la verificación del runner (`a changed file was moved to the Trash`), hacer que `ApplyGate` ignore su huella (`an apply was authorised for a plan that was never shown`), y no calcular el digest; y **un apply detenido a los 2 de 5 deja las 2 entradas en el journal**, así que el undo todavía las alcanza | cuatro roturas, entre ellas: lanzar desde el chequeo de cancelación en vez de romper el loop (`unexpected error: CancellationError()`, y el journal se queda sin las filas) |
| `folder-apply` | Mueve una carpeta a la **Papelera real** y la devuelve entera, y **rehúsa** otra que contiene un archivo que la conservada no tiene, nombrándolo; un escaneo sin revisar no planea nada y **reporta la etapa antes de trabajar**: verificar aparece antes de mover y lleva cuenta de archivos revisados | dos roturas: quitar el chequeo de contención (`2 folders moved, wanted 1`), y que un par sin decidir caiga al default del CLI (`an untouched folder review planned 2 moves`) , y dos más: quitar la emisión de verificación (`nothing was ever reported as verifying`) y emitir `.moving` antes que ella (`claimed to be moving before it verified anything`) |
...
| `cancel` | Un escaneo con un hasher **deliberadamente lento** (3 ms por archivo, 800 archivos) se interrumpe a los 6: lanza en vez de devolver resultado, se detiene en ~1 ms, no escribe documento, y deja los 800 archivos byte-idénticos | quitar los `checkCancellation` de `DuplicateFinder`: `a cancelled scan returned a result instead of throwing`. **La primera versión sin el hasher lento pasaba local y falló en CI** — era una carrera, no una prueba |
...
| `history` | Dos sesiones reales en la Papelera real: el historial las lista con sus conteos, **deshace la más vieja**, y la más nueva conserva su archivo y su undo. Y el podador coincide con la ventana, porque los dos leen las mismas filas | dos roturas: deshacer `rows.first` en vez del índice (`the older session's file did not come back`), y que `hasWorkLeft` ignore `restoredCount` (`a fully restored session is still offered an undo`) |
| `cache-liveness --cache --perceptual-cache` | Solo lectura sobre las **dos** cachés reales: cuántas filas describen un archivo que existe. La de digests midió 55.8% de desperdicio antes de que la poda existiera y **1.7% después**, sola, sobre datos reales; la perceptual mide 0%. Y afirma que las clases suman | necesita una caché real; sin `--cache` usa la default y salta si no hay |
| `video-timing --file` | Cronometra el muestreo de un video real con las dos tolerancias, con calentamiento y mejor de tres, y afirma que las dos encuentran cuadros y coinciden en la duración | sin `--file` salta diciéndolo; `scripts/video-baseline.sh` corre la mitad de ffmpeg y pone las dos lado a lado |
| `corpus --dir [--folders] [--prefix-threshold]` | Corre un escaneo real sobre un corpus sintético y llena la fila de benchmark del plan: archivos, grupos, bytes leídos, sondeos, wall, files/s, MB/s y RSS pico. Con `--folders` mide el detector de carpetas y nombra las clases truncadas; con `--similar`, el perceptual; con `--concurrency`, el barrido | los corpora los construye `scripts/make-corpus.py` con semilla fija; sin `--dir` salta |
| `network-fallback --dir` | Sobre un **montaje de red real**: la Papelera rehúsa con su razón y el archivo sigue en el share, el fallback lo mueve a una cuarentena local reportando `.quarantine`, el journal lo conserva, y el deshacer lo devuelve al share byte-idéntico. Afirma además que el volumen **no** es local, o no se probaría nada | dos roturas: apuntar el `FallbackDisposer` a la Papelera en las dos mitades, y escribir `.trash` en el journal. Sin `--dir` salta diciendo el `mount_smbfs` |

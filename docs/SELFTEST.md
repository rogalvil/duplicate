...
| `corpus --dir [--folders] [--prefix-threshold]` | Corre un escaneo real sobre un corpus sintético y llena la fila de benchmark del plan: archivos, grupos, bytes leídos, sondeos, wall, files/s, MB/s y RSS pico. Con `--folders` mide el detector de carpetas y nombra las clases truncadas | los corpora los construye `scripts/make-corpus.py` con semilla fija; sin `--dir` salta |
